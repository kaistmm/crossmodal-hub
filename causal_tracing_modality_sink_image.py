...
from logic.object import get_object_token_from_image_list
    ...
    if replace_mode == "all":
        ...
        for L in layer_range:
            for idx in rand_tokens:
                layerlist.append((idx, layername(model, L, kind)))
    elif replace_mode == "random_sink_num_half":
        k_mean = int(len(base_sink_data.keys())//2)
        if k_mean <= 0:
            return layerlist
        if k_mean <= len(base_indices):
            rand_tokens = random.sample(base_indices, k_mean) 
...
def parse_args():
    ...
    parser.add_argument("--json_path", type=str, default="qwen7b/json_files/step2/people.json",
                        help="Path of dataset json (.json)")
    parser.add_argument("--save_path", type=str, default="exp/qwen7b/step2/people",
    ...
    parser.add_argument("--strict_level", type=str, default="high")
...
if __name__ == "__main__":
    ...
    assert mode=="people","support only people mode."
    num_layers=28 
    ...
    for kind in kinds:
        for layer in layers:
                if "sink" in replace_mode:
                    ...
                    save_result_filename = os.path.join(save_path, replace_mode+f"_{strict_level}", kind+f"_layer{layer}_window{window}"+os.path.basename(json_path).replace(".jsonl", "_results.jsonl"))
                ...
                for idx, sample in enumerate(tqdm(data)):
                    ...
                    video_name=os.path.basename(os.path.dirname(sample[1]["content"][0].get("video", "unknown_video")[0]))
                    if mode=="people":
                        corrupt_audios=[a.copy() for a in audios]
                        corrupt_audios[0]=np.zeros_like(audios[0])
                        print("🍍🍍🍍 People mode : Audio corrupted!")
                        corrupt_inputs = processor(text=text, audio=corrupt_audios, images=images, videos=videos, return_tensors="pt", padding=True, use_audio_in_video=USE_AUDIO_IN_VIDEO)
                    ...
                    if "object" in replace_mode:
                        assert mode=="people"
                        print("🍍 Extracting object tokens...",strict_level)
                        object_token_list, non_object_token_list = get_object_token_from_image_list(correct_inputs,video_idx, video_path,reconstruct=False, strict_level=strict_level)
