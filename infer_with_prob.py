...
def parse_args():
    ...
    parser.add_argument("--k_divide", type=str, default=1)
...
if __name__ == "__main__":
    ...
    save_result_filename = os.path.join(
        ...
        os.path.basename(args.json_path).replace(".jsonl", "_results.jsonl")  # jsonl형태 추천
    )
    ...
    for idx, sample in enumerate(tqdm(data)):
        try:
            ...
            K=int(input_token_len/k_divide)
            print("🔔🔔🔔 Generating for idx", input_token_len,"k_divides",K) #1833
            ...
            a_prob=max_prob_for_word(result.logits, a_word, processor.tokenizer)
            v_prob=max_prob_for_word(result.logits, v_word, processor.tokenizer)
            av_prob=max_prob_for_word(result.logits, av_word, processor.tokenizer)
            if modality=="av":
                ...
                visual_sink_token, visual_non_sink_list, audio_sink_token, audio_non_sink_list= get_global_sink_token(result,video_idx,audio_idx,K=K)
                sinks = torch.cat([visual_sink_token, audio_sink_token]).cpu().numpy().tolist() #72
                v_sinks = visual_sink_token.cpu().numpy().tolist() if isinstance(visual_sink_token, torch.Tensor) else visual_sink_token
                a_sinks = audio_sink_token.cpu().numpy().tolist() if isinstance(audio_sink_token, torch.Tensor) else audio_sink_token
                num_layers=28
                history = collect_layer_stats_fast(
                    result["attentions"][0], 
                ...
                if isinstance(video_path, list):
                    video_path=os.path.basename(os.path.dirname(video_path[0]))
                sink_token_layer_history_path=os.path.join(os.path.dirname(save_result_filename),"history",f"k_divde_{k_divide}", f"{os.path.basename(video_path).split('.')[0]}_history.json")
                os.makedirs(os.path.dirname(sink_token_layer_history_path), exist_ok=True)
                with open(sink_token_layer_history_path, "w", encoding="utf-8") as f:
            ...
            result_dict = {
                ...
                "choices": sample[1].get("choices", None),
                "text": sample[1].get("text", None),
                "text_output": text_output[0],
                "av": av_word,
                "av_prob": av_prob.item(),
                "v": v_word,
                "v_prob": v_prob.item(),
                "a": a_word,
                "a_prob": a_prob.item(),
