...
from logic.runningstats import Covariance, tally
...
def trace_with_patch(
    ...
    patch_spec = defaultdict(list) #input:(11,10)
    ...
    with torch.no_grad(), nethook.TraceDict(
        ...
        outputs_exp = model.generate(**inp,use_audio_in_video=True)
    return outputs_exp #correct_answer_num,restored_answer_num,correct_answer_lang, restored_answer_lang,correct_probs, restored_probs

def trace_with_patch_memory_optimized(
    model, 
    inp_correct,inp_corrupt, 
    states_to_patch,masking_token=None
):
    # 1. 배치 분리 (Batch Slicing)
    # inp가 {'input_ids': [2, seq], ...} 형태라고 가정하고 각각 1개씩 쪼갭니다.
    # index 0: Source (값을 가져올 깨끗한 입력)
    # index 1: Target (값을 덮어씌울 손상된 입력)
    
    patch_spec = defaultdict(list)
    for t, l in states_to_patch:
        patch_spec[l].append(t)
    
    layers_to_track = list(patch_spec.keys())
    print("🍀🍀🍀  Total patching positions:", layers_to_track)
    # 값을 저장해둘 임시 저장소 (VRAM 절약을 위해 필요한 벡터만 CPU로 내릴 수도 있음)
    cached_states = defaultdict(dict)

    def untuple(x):
        return x[0] if isinstance(x, tuple) else x

    # --- Step 1: 필요한 값 미리 가져와서 저장하기 (Caching) ---
    def save_hook(x, layer):
        h = untuple(x)
        if layer in patch_spec:
            for t in patch_spec[layer]:
                if h.shape[1]==1:
                    continue
                cached_states[layer][t] = h[0, t].detach().clone()
        return x

    # Source 입력으로 한 번 돌려서 값 저장 (Gradient 계산 불필요)
    with torch.no_grad(), nethook.TraceDict(
        model,
        layers_to_track,
        edit_output=save_hook
    ):
        # generate를 쓰든 forward를 쓰든 상관없으나, 토큰 위치를 맞추기 위해 동일한 함수 사용 권장
        correct_output=model.generate(**inp_correct,output_attentions=False,output_hidden_states=False, use_audio_in_video=True)

    # --- Step 2: 저장된 값으로 끼워넣기 (Patching) ---
    def patch_hook(x, layer):
        h = untuple(x)
        # h shape: [1, seq_len, hidden_dim]
        
        if layer in cached_states:
            for t in patch_spec[layer]:
                if t in cached_states[layer]:
                    if h.shape[1]==1:
                        continue
                    h[0, t] = cached_states[layer][t]
        return x

    # Target 입력으로 다시 돌려서 결과 생성
    with torch.no_grad(), nethook.TraceDict(
        model,
        layers_to_track,
        edit_output=patch_hook
    ):
        corrupt_output = model.generate(**inp_corrupt, output_attentions=False,output_hidden_states=False,use_audio_in_video=True,masking_token=masking_token)
    return correct_output,corrupt_output
...
class ModelAndTokenizer:
    ...
    def __init__(
        ...
        self.layer_names = [
            ...
            if (re.match(r"^(thinker)\.(model)\.(layers)\.\d+$",n))
        ]
...
def layername(model, num, kind=None):
    if hasattr(model, "thinker"):
        if kind == "attn":
            kind = "self_attn"
        if kind=="before_attn":
            kind="input_layernorm"
        return f'thinker.model.layers.{num}{"" if kind is None else "." + kind}'
